		...
		createDeliveryItem: function(item)
		{
			...
			if (item.PRICE >= 0 || typeof item.DELIVERY_DISCOUNT_PRICE !== 'undefined')
			{
				costNode = BX.create('DIV', {
					...
					html: item.PRICE > 0 ? (typeof item.DELIVERY_DISCOUNT_PRICE !== 'undefined'
						? item.DELIVERY_DISCOUNT_PRICE_FORMATED
						: item.PRICE_FORMATED) : this.params.MESS_PRICE_FREE
				});
			}
			else if (deliveryCached && (deliveryCached.PRICE >= 0 || typeof deliveryCached.DELIVERY_DISCOUNT_PRICE !== 'undefined'))
			{
				costNode = BX.create('DIV', {
					props: {className: 'bx-soa-pp-delivery-cost'},
					html: item.PRICE > 0 ? (typeof deliveryCached.DELIVERY_DISCOUNT_PRICE !== 'undefined'
						? deliveryCached.DELIVERY_DISCOUNT_PRICE_FORMATED
						: deliveryCached.PRICE_FORMATED) : this.params.MESS_PRICE_FREE
				});
			}
			...
			if(bHasPeriod || bHasCalculate){
				itemNode = BX.create('DIV', {
					props: {className: 'bx-soa-pp-company bx-soa-pp-company-item' + (bHasPeriod ? ' bx-soa-pp-company--hasperiod' : '') + (bHasCalculate ? ' bx-soa-pp-company--hascalculate' : '') + ' col-lg-4 col-sm-4 col-xs-6'},
				...
				itemNode = BX.create('DIV', {
					props: {className: 'bx-soa-pp-company bx-soa-pp-company-item col-lg-4 col-sm-4 col-xs-6'},
					children: [label, costNode, title, injectNode, descNode],
					events: {click: BX.proxy(this.selectDelivery, this)}
				});
			}
